...
def plot_results(org_image, cfa, model = None, zoom=False, epoch = None, exp_path = None):
    if model is None:
        ...
        if zoom:
            ax[0].imshow(org_image.numpy()[0][:32,:32,:])
            ax[1].imshow(cfa.numpy()[0][:32,:32,:])
            ax[2].imshow(output[0][:32,:32,:])
        else:
            ax[0].imshow(org_image.numpy()[0])
            ax[1].imshow(cfa.numpy()[0])
        ...
        if exp_path is not None:
            ...
            fig.savefig(os.path.join(exp_path, f'fig_{epoch}.jpg'))
#%%
experiment_dir = '/home/aicenter/Projects/demosaicing/deep_learning_for_demosaicing/demosaicing_v2/experiments'
experiment_name = 'model_skip_con_const_kernel_'
run_id = 0
...
ax[0].imshow(images.numpy()[i])
ax[1].imshow(target.numpy()[i])


# %%
mb = ModelBuilder()
model = mb.build_model_v2()
#%%
# model.load_weights('/home/aicenter/Projects/demosaicing/deep_learning_for_demosaicing/demosaicing_v2/checkpoints_MS_v3_s1_128/cp-0015.ckpt')

plot_results(target, images, model, zoom=True, epoch=00, exp_path=exp_path)
...
viz_cb = VizCallback(get_test_data, plot_results, 5)


checkpoint_path = "checkpoints_MS_v4_s1_128/cp-{epoch:04d}.ckpt"
...
hist_df.head()
hist_df.to_csv(os.path.join(exp_path, f'history_run_{4}.csv'))

#%%
plot_results(target, images, model, epoch=15, exp_path=exp_path)

#%%
tot_psnr = 0
c = 1
model.load_weights('/home/aicenter/Projects/demosaicing/deep_learning_for_demosaicing/demosaicing_v2/checkpoints_MS_v3_s1_256/cp-0005.ckpt')
for image, target in tqdm(val_ds):
    output = model(image)
    # plot_results(target, images, model)
    # break
    for i in range(len(image)):
        psnr = calc_psnr(target[i], output[i])
        tot_psnr += psnr
        c += 1
#     break
# fig, ax = plt.subplots(1,2)
# ax[0].imshow(output.numpy()[i])
# ax[1].imshow(target.numpy()[i])
# # break
#%%
avg_psnr = tot_psnr/c
